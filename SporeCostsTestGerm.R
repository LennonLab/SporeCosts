...
singleGeneRepOpp      <- sporeTranslationOppDir$gene_length.filled*35*2
singleGeneRepDir      <- sporeTranslationOppDir$gene_length.filled*13*2
...
scSpore <- ggplot(scDataMut, aes(x = log10(sc)))+
  ...
  geom_histogram(aes(y=..density.., fill = source), alpha = 0.3) +
  geom_histogram(data = scTotal,aes(y=..density.., x = log10(sc) ), alpha = 0.3) +
  geom_density(aes(color=source), size = 1)+
  geom_density(data = scTotal, aes(y=..density.., x = log10(sc)), size = 1, color = "grey25", linetype = "dashed")+
...
ggsave("~/GitHub/sporeCostsVer2/figures/scSpore.pdf", scSpore, height = 5, width = 6)

neSpore <- ggplot(scDataMut, aes(x = Ne, y = sc))+
  ...
  ylab("Fractional costs (Sc)")+
  mytheme
  
ggsave("~/GitHub/sporeCostsVer2/figures/neSpore.pdf", neSpore, height = 5, width = 6)
...
replicationGerm   = (sum(germRepTotal))/all_Germ_cost*100 #germination genes 0.7%
transcriptionGerm = all_Germ_sum_pie$costs[1]/all_Germ_cost*100 #7.6%
translationGerm   = all_Germ_sum_pie$costs[2]/all_Germ_cost*100 #71%
...
#############################################
# Time normalized sporulation & germination 
# Costs per unit time 
############################################
# I'll use the model predictions
ttN       <- seq(0.5,8, by = 0.01)
preds     <- predict(fit, list(time = ttN))

# col names should be the same 
timeg  <- c(germination6sum_int_sum$hours, 5, 6, 7, 8)
costsg <- c(germination6sum_int_sum$costs, 0, 0, 0, 0)

germinationWill <- cbind.data.frame(time = timeg, costs = costsg)

fit3W <- nls(costs ~ SSasymp(time, yf, y0, log_alpha), data = germinationWill)
predg <- predict(fit3W, list(time = ttN))

compdqt    <- cbind.data.frame(ttN, preds, predg, ratio1 = predg/preds, ratio2 = preds/predg)

ratio <- ggplot(compdqt, aes(x = ttN, y = ratio2))+
  geom_line(size = 1)+
  mytheme+
  scale_y_continuous(sec.axis=dup_axis())+
  scale_x_continuous(sec.axis=dup_axis())+
  labs(x = "Time (h)", y = "ATP ratio")

ggsave("~/GitHub/sporeCostsVer2/figures/ratio.pdf", ratio, height = 4, width = 5)
